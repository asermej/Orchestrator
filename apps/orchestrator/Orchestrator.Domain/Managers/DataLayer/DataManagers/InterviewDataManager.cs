...
internal sealed class InterviewDataManager
{
    ...
    {
        const string sql = @"
            SELECT id, job_id, applicant_id, agent_id, interview_configuration_id, interview_guide_id, interview_template_id, token, status, interview_type, scheduled_at, started_at, completed_at, current_question_index, created_at, updated_at, is_deleted
            FROM interviews
    ...
    public async Task<Interview> Add(Interview interview)
    {
        ...
        const string sql = @"
            INSERT INTO interviews (id, job_id, applicant_id, agent_id, interview_configuration_id, interview_guide_id, interview_template_id, token, status, interview_type, scheduled_at, created_by)
            VALUES (@Id, @JobId, @ApplicantId, @AgentId, @InterviewConfigurationId, @InterviewGuideId, @InterviewTemplateId, @Token, @Status, @InterviewType, @ScheduledAt, @CreatedBy)
            RETURNING id, job_id, applicant_id, agent_id, interview_configuration_id, interview_guide_id, interview_template_id, token, status, interview_type, scheduled_at, started_at, completed_at, current_question_index, created_at, updated_at, is_deleted";
    ...
    public async Task<PaginatedResult<Interview>> Search(Guid? groupId, Guid? jobId, Guid? applicantId, Guid? agentId, string? status, int pageNumber, int pageSize)
    {
        ...
        var querySql = $@"
            SELECT i.id, i.job_id, i.applicant_id, i.agent_id, i.interview_configuration_id, i.interview_guide_id, i.interview_template_id, i.token, i.status, i.interview_type, i.scheduled_at, i.started_at, i.completed_at, i.current_question_index, i.created_at, i.updated_at, i.is_deleted
            FROM interviews i
            {joinSql}
            {whereSql}
