...
└── Orchestrator.Database/               # Database Management
...
### Orchestrator.Api (Presentation Layer)
...
### Orchestrator.AcceptanceTests (Testing Layer)
...
### Orchestrator.Database (Data Layer)
...
```bash
# List all secrets
dotnet user-secrets list --project ./apps/orchestrator/Orchestrator.Api/Orchestrator.Api.csproj

# Remove a secret
dotnet user-secrets remove "KeyName" --project ./apps/orchestrator/Orchestrator.Api/Orchestrator.Api.csproj

# Clear all secrets
dotnet user-secrets clear --project ./apps/orchestrator/Orchestrator.Api/Orchestrator.Api.csproj
```
...
The `UserSecretsId` is defined in `Orchestrator.Api.csproj`.

### Configuration Files

- `Orchestrator.Api/appsettings.json` - Default configuration (non-sensitive settings only)
- `Orchestrator.Api/appsettings.Development.json` - Development environment overrides
...
1. Create new changeset in `Orchestrator.Database/changelog/changes/`
2. Update `Orchestrator.Database/changelog/db.changelog-master.xml`
...
1. **Domain Model**: Add/update models in `Orchestrator.Domain/Managers/Models/`
...
### 📋 Database Rules (Orchestrator.Database/.cursor/rules/)
...
### 🏗️ Database Templates (Orchestrator.Database/.cursor/templates/)
...
- [Web Documentation →](Orchestrator.Web/README.md)
