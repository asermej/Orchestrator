...
1. **Group-scoped**: All test entities live under groups whose name starts with `TestOrg_`. Cleanup finds those group IDs and deletes all child rows in reverse FK order.
...
- **All test group names MUST start with `TestOrg_`** (e.g. `TestOrg_{Guid.NewGuid():N}`).
...
3. **Ensure test data uses `TestOrg_` groups** so it's scoped by the cleanup queries.
